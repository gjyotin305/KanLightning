...
model = KANFormer(vocabulary_size=len(vocab_check), hidden_size=32, num_heads=2, window_size=4, d_ff=64, num_experts=4, n_experts_per_token=2, n_blocks=6, max_seq_len=128)
...
class KANLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        out = self.kan(x)
        loss = lossfn(out, y)
        mask = loss<9 == 0 # To not include loss of padded tokens
